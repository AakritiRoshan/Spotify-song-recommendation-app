...
with stats:
    ...
    if button == "Analyze":
        ...
        if preview_box:
            st.warning(
                "Warning: some audio previews may be very loud and audio volume will reset if the page is refreshed")
        ...
        if lyrics_box:
            ...
            try:
                ...
                st.error(
                    f"Unfortunately, the Genius API could not find lyrics for the song {songs.track_name()} by {songs.track_artist_name()} - I am working on fixing this")
